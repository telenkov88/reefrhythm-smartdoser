 ...
 */
export const arrayToStringPart = (arr, unit, options) => {
 ...
 * @return The part of a cron string
 */
const toString = (values, unit, options) => {
    let retval = "";
    if (isFull(values, unit)) {
        ...
        if (step && isInterval(values, step)) {
            if (isFullInterval(values, unit, step)) {
                ...
                const range = formatValue(min, unit, options) +
                    "-" +
...
const formatValue = (value, unit, options) => {
    if ((options.outputWeekdayNames && unit.name === "weekday") ||
 ...
 * @return The `Error`
 */
const getError = (error, unit) => new Error(`${error} for ${unit.name}`);
/**
...
const parseRange = (rangeString, context, unit) => {
    ...
    if (subparts.length === 1) {
        ...
        if (maxValue < minValue) {
            throw getError(`Max range is less than min range in "${rangeString}"`, unit);
        }
 ...
 */
const replaceAlternatives = (str, unit) => {
    if (unit.alt) {
        str = str.toUpperCase();
