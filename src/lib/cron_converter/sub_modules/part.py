class Part:
    ...
    def __lt__(self, other):
        """This Part object is lower than the other Part by comparing the values list."""
        return len(self) < len(other)

    def __eq__(self, other):
        """This Part object is equal to the other Part by comparing the values list."""
        return len(self) == len(other)
    ...
    def _fix_sunday(self, values: list)->list:
    ...
    @staticmethod
    def _parse_range(unit_range: str) -> list:
    ...
    def _get_step(self, range_string_parts):
    ...
    @staticmethod
    def _apply_interval(values, step):
    ...
    def out_of_range(self, values):
        """Finds an element from values that is outside of the range of self.unit

        :param values: The values to test.
        :return: An integer if a value out of range was found, otherwise None.
        """
        first = values[0]
        last = values[-1]
    ...
    def get_step(self):
        ...
        :return: step between numbers in the interval if array interval > 2.
        """
    ...
    def to_ranges(self):
        ...
        :return: The range as a multi-dimensional array.
        """
        multi_dim_values = []
        start_number = None
        for idx, value in enumerate(self.values):
    ...
    def format_value(self, value):
        ...
        """
        if ('output_weekday_names' in self.options and self.options.get('output_weekday_names') and self.unit.get(
                'name') == 'weekday') or \
                ('output_month_names' in self.options and self.options.get('output_month_names') and self.unit.get(
                    'name') == 'month'):
